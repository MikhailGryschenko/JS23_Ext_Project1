...
const images = () => {
  ...
  workSection.addEventListener('click', e => {
    ...
    if (target && target.matches('div.popup')) {
      // если target существует и пользователь кликнул не на изображение, а на подложку - совпадение с вновь созданным div с классом popup
      imgPopup.style.display = 'none';
    }
  });
};
...
const modals = () => {
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    // Если мы не будем передавать 4-й аргумент, то модальное окно по умолчанию НЕ будет закрываться при клике на подложку
...
const tabs = function (headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  header.addEventListener('click', e => {
    const target = e.target; //тот элемент, на который кликнул пользователь
    if (target && (target.classList.contains(tabSelector.replace(/\./, "")) ||
    // с помощью регул. выражений удаляем у селектора точку и получаем класс
